    ...
    def _(self, expr: sy.Pow) -> Tuple[Expr,IndexTracker]:
        ...
        for a in expr.args:
            ...
            if len(it.free) != 0 or len(it.contracted) != 0:
                raise InvalidIndexError(repr(expr))
            #assert len(it.free) == 0, f"a:{a}, it:{it}"
            #assert len(it.contracted) == 0, f"a:{a}, it:{it}"
...
## @multimethod
## def expand_contracted_indices(xpr: sy.Add, sym: Sym) -> Expr:
##     ret: Expr = do_sympify(0)
##     for arg in xpr.args:
##         ret += expand_contracted_indices(arg, sym)
##     return ret

## @expand_contracted_indices.register
## def _(xpr: PowType, sym:Sym) -> Expr:
##     return Pow(expand_contracted_indices(xpr.args[0], sym), xpr.args[1])

## @expand_contracted_indices.register
## def _(xpr: sy.IndexedBase, sym:Sym) -> Expr:
##     return xpr

## @expand_contracted_indices.register
## def _(xpr: sy.Symbol, sym:Sym) -> Expr:
##     return xpr

## @expand_contracted_indices.register
## def _(xpr: sy.Integer, sym:Sym) -> Expr:
##     return xpr

## @expand_contracted_indices.register
## def _(xpr: sy.Indexed, sym:Sym) -> Expr:
...
## @expand_contracted_indices.register
## def _(xpr: sy.Mul, sym:Sym) -> Expr:
##     index_list = sorted(list(get_contracted_indices(xpr)), key=byname)
##     out : Expr = xpr
##     for index_item in index_list:
##         output = do_sympify(0)
##         index_values: Dict[Idx, Idx] = dict()
##         while incr([index_item], index_values):
##             output += do_subs(out, index_values, sym)
##         out = output
##     return out

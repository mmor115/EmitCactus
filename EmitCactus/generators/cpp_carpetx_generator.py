...
class CppCarpetXGenerator(CactusGenerator):
    ...
    def generate_schedule_ccl(self) -> ScheduleRoot:
        ...
        for fn_name, fn in sorted(self.thorn_def.thorn_functions.items()):
            ...
            #  the trick due to dependency issues with symbols defined in the recipe. Instead, we will just duplicate
            #  all Evolve and Analysis thorn functions into PostPostInit to ensure everything is initialized.
            if fn.schedule_target in [ScheduleBin.Evolve, ScheduleBin.SpecialEvolve, ScheduleBin.Analysis]:
                post_init_bin, post_init_at_in = self._resolve_schedule_target(ScheduleBin.PostPostInit)
